...
const UserManagement: React.FC<UserManagementProps> = ({ teachers, users, onUpdateTeacher, onViewTeacher, onViewStudentDashboard, defaultCoverImages }) => {
    ...
    const [activeTab, setActiveTab] = useState<'teachers' | 'approved_teachers' | 'students'>('teachers');
    ...
    const [manualFeeRates, setManualFeeRates] = useState<{ [key: string]: string }>({});
    ...
    const sortedTeachers = useMemo(() => {
        ...
        let filteredTeachers = [...teachers].filter(t => !t.isManaged); // Exclude managed teachers from admin view
    ...
    const VerificationStatusIcon: React.FC<{ status: VerificationStatus }> = ({ status }) => {
        const statusMap: Record<string, { icon: React.ReactNode, text: string }> = {
        ...
        const { icon, text } = statusMap[status] || statusMap.unverified;
    ...
    const StatusBadge: React.FC<{ status: Teacher['registrationStatus'] }> = ({ status }) => {
        ...
        return <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${style}`}>{safeStatus.charAt(0).toUpperCase() + safeStatus.slice(1)}</span>;
    };
                        ...
                        return (
                            <tr key={teacher.id}>
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-center">
                                    <span className={`font-bold ${profileCompletion === 100 ? 'text-green-600 dark:text-green-400' :
