...
import { Teacher, Sale, User, Withdrawal, PayoutDetails } from '../../types.ts';
...
const EarningsDashboard: React.FC<EarningsDashboardProps> = ({ teacher, allSales, allUsers, isAdminView, onWithdraw, onSaveBankDetails, onVerificationUpload, onVerificationDecision, onUpdatePhysicalOrderStatus }) => {
    const { processMonthlyPayouts, tuitionInstitutes } = useData();
    ...
    const teacherSales = useMemo(() => {
        return allSales
            ...
            .map(sale => ({ ...sale, student: allUsers.find(u => u.id === sale.studentId) }));
    ...
    const { lastMonthPayout, payoutDate, payoutStatus } = useMemo(() => {
        ...
        const lastMonthSales = teacherSales.filter(sale => {
    ...
    }, [teacherSales, teacher.commissionRate, teacher.earnings.processedPayouts]);
    ...
    const filteredSales = useMemo(() => {
        ...
        let filtered = teacherSales.filter(sale => {
        ...
        return filtered;
    }, [teacherSales, salesSearchTerm, salesFilter, salesDateFilter, sortConfig]);
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md">
                <h2 className="text-xl font-bold mb-4 text-light-text dark:text-dark-text">Sales History</h2>
            ...
            {viewingAddress && viewingAddress.shippingAddress && (
                <Modal isOpen={!!viewingAddress} onClose={() => setViewingAddress(null)} title="Shipping Details">
