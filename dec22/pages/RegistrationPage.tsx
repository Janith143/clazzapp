...
const RegistrationPage: React.FC<RegistrationPageProps> = ({ refCode, googleUser, firebaseUser, initialMethod, prefillData }) => {
  ...
  const { handleNavigate, teacherCardTaglines } = useNavigation();
  ...
  const createProfile = async (
      ...
      if (newUser.role === 'teacher') {
          const generateUniqueUsername = (fullName: string, allTeachers: Teacher[]): string => {
            // From "NImanthika Mullegama" to "nimanthikamullegama"
            const baseSlug = fullName.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9]/g, '');
            const allUsernames = new Set(allTeachers.map(t => t.username));
            
            if (!allUsernames.has(baseSlug)) {
                return baseSlug;
            }
            
            // Handle collisions by appending numbers, e.g., "nimanthikamullegama2"
            let counter = 2;
          ...
          const newTeacher: Teacher = {
              ...
              coverImages: [], // Initialize as empty. The UI will handle showing defaults.
      ...
      } else if (newUser.role === 'tuition_institute') {
          const newInstitute: TuitionInstitute = {
              id: newUser.id,
              userId: newUser.id,
              name: instituteName!,
          ...
          await addTuitionInstitute(newInstitute);
      }
      return newUser;
  };
