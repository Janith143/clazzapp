...
const CourseEditorPage: React.FC<CourseEditorPageProps> = ({ courseData, teacherId, onSave, onCancel }) => {
  ...
  // Get the teacher object to access teachingItems and legacy subjects
  ...
  const legacySubjects = teacher?.subjects || [];
  const hasLegacySubjects = legacySubjects.length > 0;

  const defaultSubject = hasTeachingItems 
      ? teachingItems[0].subject 
      : (hasLegacySubjects ? legacySubjects[0] : (allSubjects[0]?.value || ''));
  ...
  const subjectOptions = useMemo(() => {
      ...
      } else if (hasLegacySubjects) {
          // If no structured teaching items, fallback to legacy subjects list
          options = legacySubjects.map(s => ({ value: s, label: s }));
      } else {
          // If no subjects in profile at all, fallback to all system subjects
          options = allSubjects;
      }
      
      // Ensure current selected subject is in the list to support legacy data/edits
      if (course.subject && !options.some(s => s.value === course.subject)) {
           options.push({ value: course.subject, label: course.subject });
      }
      return options;
  }, [hasTeachingItems, teachingItems, hasLegacySubjects, legacySubjects, allSubjects, course.subject]);
